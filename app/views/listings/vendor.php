<?php $title = 'admin | ' . $this->e($admin->name) .' | '.$_SERVER['HTTP_HOST']  ?>

<div class="large-12 columns">
    <h3 class="subheader">admin <?= $this->e($admin->name) ?></h3>
    ...
    <?php if(empty($feedbacks)): ?>
        <div data-alert class="alert-box secondary">
            This admin has no feedbacks yet.
        </div>
    <?php else: ?>
    ...
    <h4 class="subheader">Tickets</h4>
    <?php if(empty($tickets)): ?>
        <div data-alert class="alert-box secondary">
            This admin has no public tickets.
        </div>
    <?php else: ?>
        <?php
        $ticketsPerRow = 5;
        $withadmin = false;
        require '../app/views/listings/_ticket_list.php';
        ?>
    <?php endif ?>
